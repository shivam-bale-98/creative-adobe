...
class Form
{
    ...
    /**
     * Renders a multiple select checkbox.
     *
     ...
     * @return $html
     */
    public function selectMultipleCheckbox($key, $optionValues, $defaultValues = false, $miscFields = [])
    {
        ...
        $str = "<div style='height: 350px; overflow-y: scroll;' id=\"$key\" name=\"{$key}[]\" multiple=\"multiple\"" . $this->parseMiscFields('form-control', $miscFields) . '>';
        foreach ($optionValues as $k => $text) {
            $str .= '<div class="checkbox">
                        <label>
				            <input type="checkbox" class="ccm-input-checkbox" value="' . h($k) . '" ';
