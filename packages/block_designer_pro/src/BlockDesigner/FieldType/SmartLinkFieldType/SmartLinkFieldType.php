...
if (trim($' . $slugURL . ') != "") { ?>' . $this->data['prefix'] . '<?php
    ...
    {
        $parent = $this->getRepeating() && isset($this->data['parent']) ? $this->data['parent']['slug'] . '-' : '';
	...
	public function getRepeatableUpdateItemJS()
	{
        ...
        $html = '    $.each($(newField).find(\'.ft-smart-link\'), function () {
            var ftSmartLinkImage = $(this).find(\'.' . $this->linkTypeClass('image') . '\');
            ...
            var ftSmartLinkFile = $(this).find(\'.' . $this->linkTypeClass('file') . '\');
            ...
            var ftSmartLinkPage = $(this).find(\'.' . $this->linkTypeClass('page') . '\');
	...
	$(\'#<?php echo $' . $this->data['slug'] . '_ContainerID; ?> .ft-smart-link-type\').trigger(\'change\');
		...
		if (in_array('page', $options)) {
			$optionsHTML[] = '<div class="form-group hidden d-none" data-link-type="page">
            ...
            ' . parent::generateFormContent('page_selector', ['slug' => $this->data['slug'] . '_Page', 'parent' => isset($this->data['parent']) ? $this->data['parent'] : null, 'attributes' => ['class' => $this->linkTypeClass('page')]], $repeating) . '
		</div>';
		}
		if (in_array('url', $options)) {
			$optionsHTML[] = '<div class="form-group hidden d-none" data-link-type="url">
		...
		if (in_array('relative_url', $options)) {
			$optionsHTML[] = '<div class="form-group hidden d-none" data-link-type="relative_url">
		...
		if (in_array('file', $options)) {
			$fileGroup = null;
			$fileGroup .= '<div class="form-group hidden d-none" data-link-type="file">';
			if (!$repeating) {
            ...
            ' . parent::generateFormContent('required', [], $repeating) . '
            ' . parent::generateFormContent('file', ['slug' => $slugFile, 'parent' => isset($this->data['parent']) ? $this->data['parent'] : null, 'postName' => $this->data['slug'] . '_File', 'bf' => $objectFile, 'attributes' => ['class' => $this->linkTypeClass('file')]], $repeating) . '	
		</div>';
			$optionsHTML[] = $fileGroup;
		}
		if (in_array('image', $options)) {
			$imageGroup = null;
			$imageGroup .= '<div class="form-group hidden d-none" data-link-type="image">';
			if (!$repeating) {
            ...
            ' . parent::generateFormContent('required', [], $repeating) . '
            ' . parent::generateFormContent('image', ['slug' => $slugImage, 'parent' => isset($this->data['parent']) ? $this->data['parent'] : null, 'postName' => $this->data['slug'] . '_Image', 'bf' => $objectImage, 'attributes' => ['class' => $this->linkTypeClass('image')]], $repeating) . '
		</div>';
...
' . $js;
	...
	public function validate()
	{
		...
		return count($options) >= 2 ? true : t("There need to be at least 2 link options for field '%s' to chose from, otherwise use a default field type instead.", $this->data['label'] && $this->data['label'] != '' ? $this->data['label'] : $this->data['slug']);
	}
