...
class BlockDesignerProcessor
{
	...
	public static function getFieldTypeVariable($ftHandle, $key){
		return isset(self::$variables[$ftHandle][$key]) ? self::$variables[$ftHandle][$key] : null;
	}

    public static function getPackageFolder($pkgHandle)
    {
    ...
    public static function get_class_from_file($path_to_file)
    {
        ...
        // Specifically for PHP 8 and up
        if (version_compare(phpversion(), '8.0', '>')) {
            $tokens =  PhpToken::tokenize($contents);
            $namespace = $tokens[3]->text;
            // Hack to take the last namespace section as class name
            $class = array_reverse(explode('\\', $namespace))[0];
                    ...
                    if (!$fieldRepeating) {
                        ...
                        if (method_exists($fieldTypeClass, 'validate') && ($validateResult = $fieldTypeClass->validate()) !== true) {
	                        if (is_array($validateResult)) {
		                        self::$errors = array_merge(self::$errors, $validateResult);
	                        } else {
            ...
            foreach ($assets as $assetFunctionName => $assetArray) {
                ...
                foreach ($assetArray as $assetEntriesType => $assetEntries) {
                    foreach ($assetEntries as $assetEntry) {
	                    ...
	                    $assetEntry['filename'] = str_replace('{{blockHandle}}', '\' . $this->btHandle . \'', isset($assetEntry['filename']) ? $assetEntry['filename'] : '');
                        $assetCode = null;
...
echo Core::make(\'helper/concrete/ui\')->tabs($tabs); ?>';
                if (!empty($tabsFields)) {
                    $formFields[] = '<div class="tab-content">';
                    $active = false;
                    foreach ($tabsFields as $k => $v) {
                        $formFields[] = '<div role="tabpanel" class="tab-pane'. (!$active ? ' show active' : '') .'" id="' . $k . '-<?php echo $identifier_getString; ?>">
    ' . implode('', $v) . '
</div>';
                        $active = true;
                    }
...
' . self::getTabs(1) . (!empty($controllerFunctionsArray) ? PHP_EOL . self::getTabs(1) . implode(PHP_EOL . PHP_EOL . self::getTabs(1), $controllerFunctionsArray) : null) . '
	...
	protected static function varExportArray($array, $arrayStrip = false){
    ...
    protected static function sortArrayByArray(array $array, array $orderArray)
    {
        $ordered = [];
    ...
    public static function buildSchema($tables = [])
    {
    ...
    private static function getTabs($count = 1)
    {
        $return = '';
        if ($count >= 1) {
            for ($i = 1; $i <= $count; $i++) {
