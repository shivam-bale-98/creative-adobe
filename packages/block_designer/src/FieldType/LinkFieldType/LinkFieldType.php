...
?>' . $this->data['suffix'] . '<?php } ?>';
	...
	public function getValidateFunctionContents()
	{
		if ($this->getRepeating()) {
			...
			return 'if ((in_array("' . $this->data['slug'] . '", ' . $btFieldsRequired . ') || (isset(' . $slug . ') && trim(' . $slug . ') != \'\')) && (trim(' . $slug . ') == "" || (($page = Page::getByID(' . $slug . ')) && $page->error !== false))) {
                            $e->add(t("The %s field is required (%s, row #%s).", t("' . h($this->data['label']) . '"), t("' . h($this->data['parent']['label']) . '"), $' . $this->data['parent']['slug'] . '_k));
                        }';
		} else {
			return 'if (in_array("' . $this->data['slug'] . '", $this->btFieldsRequired) && (trim($args["' . $this->data['slug'] . '"]) == "" || (($page = Page::getByID($args["' . $this->data['slug'] . '"])) && $page->error !== false))) {
    ...
    ' . parent::generateFormContent('page_selector', ['slug' => $this->data['slug'], 'parent' => isset($this->data['parent']) ? $this->data['parent'] : null, 'attributes' => ['class' => 'ft-link-' . ($repeating ? $this->data['parent']['slug'] . '-' : '') . $this->data['slug'] . '-page-selector']], $repeating) . '
</div>';
		if (!isset($this->data['hide_title']) || $this->data['hide_title'] != '1') {
	...
	{
        return 'var pageSelector = $(newField).find(\'.ft-link-'. $this->data['parent']['slug'] . '-' . $this->data['slug'] . '-page-selector' . '\');
