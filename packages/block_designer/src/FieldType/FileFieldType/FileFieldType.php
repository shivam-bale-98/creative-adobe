...
if (isset($' . $this->data['slug'] . ') && $' . $this->data['slug'] . ' > 0) {
    $' . $this->data['slug'] . '_o = File::getByID($' . $this->data['slug'] . ');
		...
		$attributes = [];
		if ($repeating) {
			$attributes['class'] = 'ccm-file-selector ft-file-' . $this->data['parent']['slug'] . '-' . $this->data['slug'] . '-file-selector';
		}
		$html .= '
...
<div class="form-group">
	...
	public function getRepeatableUpdateItemJS()
	{
        ...
        return 'var ' . $slug . ' = $(newField).find(\'.ft-file-' . $this->data['parent']['slug'] . '-' . $this->data['slug'] . '-file-selector\');
            if ($(' . $slug . ').length > 0) {
