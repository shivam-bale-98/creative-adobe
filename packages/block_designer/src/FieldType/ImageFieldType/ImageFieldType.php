...
class ImageFieldType extends FieldType
{
    ...
    public function getRepeatableUpdateItemJS()
    {
        ...
        return 'var ' . $slug . ' = $(newField).find(\'.ft-image-' . $this->data['parent']['slug'] . '-' . $this->data['slug'] . '-file-selector\');
            if ($(' . $slug . ').length > 0) {
            ...
            var ' . $slugPage . ' = $(newField).find(\'.ft-image-' . $this->data['parent']['slug'] . '-' . $this->data['slug'] . '-page-selector\');
    ...
    ' . parent::generateFormContent('image', ['slug' => $this->data['slug'], 'parent' => isset($this->data['parent']) ? $this->data['parent'] : null, 'attributes' => ['class' => 'ccm-file-selector ft-image-' . ($repeating ? $this->data['parent']['slug'] . '-' : '') . $this->data['slug'] . '-file-selector']], $repeating) . '
</div>';
        } else {
    ...
    ' . parent::generateFormContent('page_selector', ['slug' => $this->data['slug'] . '_link', 'parent' => isset($this->data['parent']) ? $this->data['parent'] : null, 'attributes' => array('class' => 'ft-image-' . ($repeating ? $this->data['parent']['slug'] . '-' : '') . $this->data['slug'] . '-page-selector')], $repeating) . '
</div>';
                    break;
                case '2':
                    $html .= PHP_EOL . '<div class="form-group">
    ...
    public function getSaveFunctionContents()
    {
        ...
        return '$args[\'' . $this->data['slug'] . '\'] = isset($args[\'' . $this->data['slug'] . '\']) && is_numeric($args[\'' . $this->data['slug'] . '\']) ? $args[\'' . $this->data['slug'] . '\'] : 0;';
    }
