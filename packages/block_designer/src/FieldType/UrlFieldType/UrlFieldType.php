...
class UrlFieldType extends FieldType
{
	...
	{
		$title = !isset($this->data['hide_title']) || $this->data['hide_title'] != '1';
		$repeating = $this->getRepeating();
		if ($repeating) {
			$slug = '$' . $this->data['parent']['slug'] . '_item_v["' . $this->data['slug'] . '"]';
			$slugTitle = '$' . $this->data['parent']['slug'] . '_item_v["' . $this->data['slug'] . '_text"]';
			if ($title) {
				return 'if (isset(' . $slugTitle . ') && trim(' . $slugTitle . ') != "") {
				$content[] = ' . $slugTitle . ';
			} elseif (isset(' . $slug . ') && trim(' . $slug . ') != "") {
                $content[] = ' . $slug . ';
            }';
			}
			...
			$slug = '$this->' . $this->data['slug'];
			$slugTitle = '$this->' . $this->data['slug'] . '_text';
			if ($title) {
				return 'if (trim(' . $slugTitle . ') != "") {
			$content[] = ' . $slugTitle . ';
		} else {
			$content[] = ' . $slug . ';
		}';
