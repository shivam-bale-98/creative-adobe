...
<?php use Concrete\Core\Support\Facade\Application;?>
...
<div class="ccm-ui ccm-search-fields-advanced-dialog" data-dialog="advanced-search">
    ...
    <?php
        ...
        echo Application::getFacadeApplication()->make('helper/concrete/ui')->tabs($tabs);
    ?>
    <form data-form="advanced-search" method="<?=$controller->getSubmitMethod()?>" action="<?=$controller->getSubmitAction()?>">

        <?=$form->hidden('fSearchFormID', $controller->getSearchProvider()->getFormID()); ?>

        <div class="tab-content">
            <div class="tab-pane active" id="tab-content-fields">
                <p class="mb-0">
                    <small class="text-muted">
                        <?=t('Add custom search fields based on your needs.')?>
                    </small>
                </p>
                <?php print $searchFieldSelectorElement->render(); ?>
            </div>

            <div class="tab-pane" id="tab-content-columns">
                <p>
                    <small class="text-muted">
                        <?=t('Customize the columns, column order, sort and number of results for your search.')?>
                    </small>
                </p>
                <?php print $customizeElement->render(); ?>
            </div>
                                ...
                                <?php foreach ($searchPresets as $searchPreset) { ?>
                                    <tr>
                                        <td style="padding-left: 15px;">
                                            <a href="<?= $controller->getSavedSearchBaseURL($searchPreset); ?>">
                                                <?= !empty($searchPreset->getPresetName())?$searchPreset->getPresetName():t('(no name set)'); ?>
                                            </a>
                                        </td>
                                        <td valign="top" style="text-align: right; padding-right: 15px;">
                                            <div class="btn-group">
                                                <button type="button" data-button-action="edit-search-preset" dialog-title="<?= t('Edit Search Preset'); ?>" data-tree-action-url="<?= $controller->getSavedSearchEditURL($searchPreset); ?>" data-toggle="tooltip" data-title="<?= t('Edit'); ?>" class="btn btn-info btn-xs">
                                                    <?=t('Edit');?>
                                                </button>
                                                <button type="button" data-button-action="delete-search-preset" dialog-title="<?= t('Delete Search Preset'); ?>" data-tree-action-url="<?= $controller->getSavedSearchDeleteURL($searchPreset); ?>" data-toggle="tooltip" data-title="<?= t('Delete'); ?>" class="btn btn-danger btn-xs">
                                                    <?=t('Delete');?>
                                                </button>
                                            </div>
                                        </td>
