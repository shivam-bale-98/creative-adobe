...
foreach ($pages as $page) {
    ...
    $title = $page->getTitle();
    $url = $page->getUrl();
    //$img = $page->getImageByAttributeHandle('case_study_img');
    $image = $page->getThumbnailImage(650, 600);
...
?>
    ...
    <?php if ($count == 5) { ?>
        <div class="case-study-card w-full parallax" data-parallax="-40">
            <a tabindex="0" role="link" aria-label="<?php echo $title; ?>" href="<?php echo $url ?>" class="flex flex-col">
                <div class="subtitle relative md:mb-[3rem] mb-[2rem]">
                    <span class="dot absolute h-[0.7rem] w-[0.7rem] bg-red-berry rounded-full"></span>
                    <p class="relative"><?php echo GeneralHelper::getTwoDigitCount($counter);  ?></p>
                </div>
                ...
                <div class="img-wrap relative rounded-[2rem] overflow-hidden md:mb-[3rem] mb-[2rem] z-2 blurred-img" style="background-image: url('<?php echo $low; ?>');">
                    <img class="absolute inset-0 size-full z-1 object-cover | lazy" loading="lazy" src="<?php echo $image; ?>" alt="<?php echo $title; ?>">
