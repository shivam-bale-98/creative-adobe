Concrete.event.bind('btSliderBlock.link.open', function (options, settings) {
...
Concrete.event.bind('slider_block.pageType.multiple_select', function () {
    $(document).ready(function () {
        $('.ft-multipleSelect-slider_block-pageType').select2();
    });
});

Concrete.event.bind('btSliderBlock.filter.edit.open', function (options, settings) {
    var container = $('#' + settings.id);
    ...
    var defaultValues = sortableItems.attr("data-default-values");
    var token = function () {
    ...
    $(container).on('change', '.attribute', function (e) {
        e.preventDefault();
        let element = $(e.target);
        let attributeHandle = element.val();
        $.ajax({
            dataType: "json",
            type: "GET",
            cache: false,
            data: {attributeHandle},
            url: CCM_DISPATCHER_FILENAME + '/api/v1/blocks/slider_block',
            success: (response) => {
                element.closest(".sortable-item-inner").find(".value").html(response.data);
            },
        });
    });
});