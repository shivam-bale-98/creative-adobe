Concrete.event.bind('btTechnologyCardSlider.addCards.edit.open', function (options, settings) {
    ...
    var ftImageImage = $(newField).find('.ft-image-addCards-image-file-selector');
            if ($(ftImageImage).length > 0) {
                Concrete.Vue.activateContext('cms', function (Vue, config) {
                    new Vue({
                        el: '#' + $(ftImageImage).attr('id'),
                        components: config.components
                    });
                });
            }
            var pageSelectorImage = $(newField).find('.ft-image-addCards-image-page-selector');
            if ($(pageSelectorImage).length > 0) {
                Concrete.Vue.activateContext('cms', function (Vue, config) {
                    new Vue({
                        el: '#' + $(pageSelectorImage).attr('id'),
                        components: config.components
                    });
                });
            }
    var ftImageIcon = $(newField).find('.ft-image-addCards-icon-file-selector');
            if ($(ftImageIcon).length > 0) {
                Concrete.Vue.activateContext('cms', function (Vue, config) {
                    new Vue({
                        el: '#' + $(ftImageIcon).attr('id'),
                        components: config.components
                    });
                });
            }
            var pageSelectorIcon = $(newField).find('.ft-image-addCards-icon-page-selector');
            if ($(pageSelectorIcon).length > 0) {
                Concrete.Vue.activateContext('cms', function (Vue, config) {
                    new Vue({
                        el: '#' + $(pageSelectorIcon).attr('id'),
                        components: config.components
                    });
                });
            }
