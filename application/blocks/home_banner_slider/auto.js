Concrete.event.bind('btHomeBannerSlider.bannerItems.edit.open', function (options, settings) {
    ...
    var ftImageDimage = $(newField).find('.ft-image-bannerItems-Dimage-file-selector');
            if ($(ftImageDimage).length > 0) {
                Concrete.Vue.activateContext('cms', function (Vue, config) {
                    new Vue({
                        el: '#' + $(ftImageDimage).attr('id'),
                        components: config.components
                    });
                });
            }
            var pageSelectorDimage = $(newField).find('.ft-image-bannerItems-Dimage-page-selector');
            if ($(pageSelectorDimage).length > 0) {
                Concrete.Vue.activateContext('cms', function (Vue, config) {
                    new Vue({
                        el: '#' + $(pageSelectorDimage).attr('id'),
                        components: config.components
                    });
                });
            }
    var ftImageMimage = $(newField).find('.ft-image-bannerItems-Mimage-file-selector');
            if ($(ftImageMimage).length > 0) {
                Concrete.Vue.activateContext('cms', function (Vue, config) {
                    new Vue({
                        el: '#' + $(ftImageMimage).attr('id'),
                        components: config.components
                    });
                });
            }
            var pageSelectorMimage = $(newField).find('.ft-image-bannerItems-Mimage-page-selector');
            if ($(pageSelectorMimage).length > 0) {
                Concrete.Vue.activateContext('cms', function (Vue, config) {
                    new Vue({
                        el: '#' + $(pageSelectorMimage).attr('id'),
                        components: config.components
                    });
                });
            }
    blockDesignerEditor('#' + ($(newField).find('textarea.ft-bannerItems-title').attr('id')).replace(/(:|\.|\[|\]|,|=|@)/g, "\\$1"));
    var pageSelector = $(newField).find('.ft-link-bannerItems-ctaLink-page-selector');
        ...
        if (deleteIt === true) {
            ...
            $(sortableItem).slideUp(slideSpeed, function () {
                if(typeof CKEDITOR !== 'undefined'){
                    var blockDesignerEditorInstance = CKEDITOR.instances[$(sortableItem).find('textarea.ft-bannerItems-title').attr('id')];
