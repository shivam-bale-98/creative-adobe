Concrete.event.bind('btOurClients.link.open', function (options, settings) {
...
});Concrete.event.bind('btOurClients.logos.edit.open', function (options, settings) {
    var container = $('#' + settings.id);
    ...
    var ftImageDesktopLogos = $(newField).find('.ft-image-logos-desktopLogos-file-selector');
            if ($(ftImageDesktopLogos).length > 0) {
                Concrete.Vue.activateContext('cms', function (Vue, config) {
                    new Vue({
                        el: '#' + $(ftImageDesktopLogos).attr('id'),
                        components: config.components
                    });
                });
            }
            var pageSelectorDesktopLogos = $(newField).find('.ft-image-logos-desktopLogos-page-selector');
            if ($(pageSelectorDesktopLogos).length > 0) {
                Concrete.Vue.activateContext('cms', function (Vue, config) {
                    new Vue({
                        el: '#' + $(pageSelectorDesktopLogos).attr('id'),
                        components: config.components
                    });
                });
            }
    var ftImageMobileLogos = $(newField).find('.ft-image-logos-mobileLogos-file-selector');
            if ($(ftImageMobileLogos).length > 0) {
                Concrete.Vue.activateContext('cms', function (Vue, config) {
                    new Vue({
                        el: '#' + $(ftImageMobileLogos).attr('id'),
                        components: config.components
                    });
                });
            }
            var pageSelectorMobileLogos = $(newField).find('.ft-image-logos-mobileLogos-page-selector');
            if ($(pageSelectorMobileLogos).length > 0) {
                Concrete.Vue.activateContext('cms', function (Vue, config) {
                    new Vue({
                        el: '#' + $(pageSelectorMobileLogos).attr('id'),
                        components: config.components
                    });
                });
            }
