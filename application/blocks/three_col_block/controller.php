<?php namespace Application\Block\ThreeColBlock;
...
class Controller extends BlockController
{
    ...
    protected $btExportTables = ['btThreeColBlock', 'btThreeColBlockAddCardsEntries'];
    protected $btTable = 'btThreeColBlock';
    ...
    public function getSearchableContent()
    {
        ...
        $addCards_items = $db->fetchAll('SELECT * FROM btThreeColBlockAddCardsEntries WHERE bID = ? ORDER BY sortOrder', [$this->bID]);
    ...
    {
        $db = Database::connection();
        $this->set('sectionTitle', LinkAbstractor::translateFrom($this->sectionTitle));
        $addCards = [];
        $addCards_items = $db->fetchAll('SELECT * FROM btThreeColBlockAddCardsEntries WHERE bID = ? ORDER BY sortOrder', [$this->bID]);
        foreach ($addCards_items as $addCards_item_k => &$addCards_item_v) {
    ...
    {
        $db = Database::connection();
        $db->delete('btThreeColBlockAddCardsEntries', ['bID' => $this->bID]);
        parent::delete();
    }
    ...
    public function duplicate($newBID)
    {
        ...
        foreach ($addCards_items as $addCards_item) {
            ...
            $db->insert('btThreeColBlockAddCardsEntries', $addCards_item);
        }
    ...
    public function edit()
    {
        ...
        $this->set('sectionTitle', LinkAbstractor::translateFromEditMode($this->sectionTitle));
        $addCards = $this->get('addCards');
        $addCards_items = $db->fetchAll('SELECT * FROM btThreeColBlockAddCardsEntries WHERE bID = ? ORDER BY sortOrder', [$this->bID]);
        foreach ($addCards_items as &$addCards_item) {
    ...
    protected function addEdit()
    {
        ...
        $al->register('css', 'repeatable-ft.form', 'blocks/three_col_block/css_form/repeatable-ft.form.css', [], $this->pkg);
        $al->register('javascript', 'handlebars', 'blocks/three_col_block/js_form/handlebars-v4.0.4.js', [], $this->pkg);
        $al->register('javascript', 'handlebars-helpers', 'blocks/three_col_block/js_form/handlebars-helpers.js', [], $this->pkg);
    ...
    public function save($args)
    {
        ...
        $args['sectionTitle'] = LinkAbstractor::translateTo($args['sectionTitle']);
        $rows = $db->fetchAll('SELECT * FROM btThreeColBlockAddCardsEntries WHERE bID = ? ORDER BY sortOrder', [$this->bID]);
                    ...
                    switch ($type) {
                        case 'update':
                            foreach ($values as $id => $data) {
                                $db->update('btThreeColBlockAddCardsEntries', $data, ['id' => $id]);
                            }
                            break;
                        case 'insert':
                            foreach ($values as $data) {
                                $db->insert('btThreeColBlockAddCardsEntries', $data);
                            }
                            break;
                        case 'delete':
                            foreach ($values as $value) {
                                $db->delete('btThreeColBlockAddCardsEntries', ['id' => $value]);
                            }
                            break;
                    }
                }
    ...
    public function validate($args)
    {
        ...
        if (in_array("sectionTitle", $this->btFieldsRequired) && (trim($args["sectionTitle"]) == "")) {
            $e->add(t("The %s field is required.", t("section title")));
        }
