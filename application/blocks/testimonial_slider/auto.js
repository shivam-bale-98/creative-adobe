Concrete.event.bind('btTestimonialSlider.testimonials.edit.open', function (options, settings) {
    ...
    var ftImageRedLogo = $(newField).find('.ft-image-testimonials-redLogo-file-selector');
            if ($(ftImageRedLogo).length > 0) {
                Concrete.Vue.activateContext('cms', function (Vue, config) {
                    new Vue({
                        el: '#' + $(ftImageRedLogo).attr('id'),
                        components: config.components
                    });
                });
            }
            var pageSelectorRedLogo = $(newField).find('.ft-image-testimonials-redLogo-page-selector');
            if ($(pageSelectorRedLogo).length > 0) {
                Concrete.Vue.activateContext('cms', function (Vue, config) {
                    new Vue({
                        el: '#' + $(pageSelectorRedLogo).attr('id'),
                        components: config.components
                    });
                });
            }
    var ftImageWhiteLogo = $(newField).find('.ft-image-testimonials-whiteLogo-file-selector');
            if ($(ftImageWhiteLogo).length > 0) {
                Concrete.Vue.activateContext('cms', function (Vue, config) {
                    new Vue({
                        el: '#' + $(ftImageWhiteLogo).attr('id'),
                        components: config.components
                    });
                });
            }
            var pageSelectorWhiteLogo = $(newField).find('.ft-image-testimonials-whiteLogo-page-selector');
            if ($(pageSelectorWhiteLogo).length > 0) {
                Concrete.Vue.activateContext('cms', function (Vue, config) {
                    new Vue({
                        el: '#' + $(pageSelectorWhiteLogo).attr('id'),
                        components: config.components
                    });
                });
            }
    blockDesignerEditor('#' + ($(newField).find('textarea.ft-testimonials-description_1').attr('id')).replace(/(:|\.|\[|\]|,|=|@)/g, "\\$1"));
    };
        ...
        if (deleteIt === true) {
            ...
            $(sortableItem).slideUp(slideSpeed, function () {
                if(typeof CKEDITOR !== 'undefined'){
                    var blockDesignerEditorInstance = CKEDITOR.instances[$(sortableItem).find('textarea.ft-testimonials-description_1').attr('id')];
