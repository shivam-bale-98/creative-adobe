...
<section <?php if (isset($bgColor) && trim($bgColor) != "") { ?>style="background-color:  <?php echo h($bgColor); ?>;" <?php } ?> class="<?php if (isset($hideBlock) && trim($hideBlock) == 1) { ?>hide-block<?php } ?> <?php if (isset($removePaddingTop) && trim($removePaddingTop) == 1) { ?>pt-0<? } ?> <?php if (isset($removePaddingBottom) && trim($removePaddingBottom) == 1) { ?>pb-0<? } ?> two-col-list with-address relative xxl:px-[10rem] xl:px-[6rem] md:px-[4rem] px-[2rem] xl:py-[10rem] md:py-[8rem] py-[5rem] text-rustic-red">
					...
					?>
						<div class="image left">
							<div class="img-wrap relative blurred-img rounded-[2rem] overflow-hidden" style="background-image: url('<?php echo $thumbnail ?>');">
								<img src="<?php echo $thumbnail ?>" alt="<?php echo h($list_item["title"]); ?>">
							</div>
						</div>
					<?php } ?>
				...
				<div class="wrp flex flex-wrap justify-between items-center">
					...
					<div class="content left">
						<?php if (isset($list_item["title"]) && trim($list_item["title"]) != "") { ?>
							...
							<h2 class="xl:mb-[4rem] mb-[2rem]"><?php echo h($list_item["title"]); ?></h2>
						<?php } ?>
