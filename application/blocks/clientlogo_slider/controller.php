<?php namespace Application\Block\ClientlogoSlider;
...
class Controller extends BlockController
{
    ...
    protected $btExportTables = ['btClientlogoSlider', 'btClientlogoSliderAddCardsEntries'];
    protected $btTable = 'btClientlogoSlider';
    ...
    public function getSearchableContent()
    {
        ...
        $addCards_items = $db->fetchAll('SELECT * FROM btClientlogoSliderAddCardsEntries WHERE bID = ? ORDER BY sortOrder', [$this->bID]);
        foreach ($addCards_items as $addCards_item_k => $addCards_item_v) {
            ...
            if (isset($addCards_item_v["externalLogoUrl"]) && trim($addCards_item_v["externalLogoUrl"]) != "") {
                $content[] = $addCards_item_v["externalLogoUrl"];
            }
        }
    ...
    {
        $db = Database::connection();
        $db->delete('btClientlogoSliderAddCardsEntries', ['bID' => $this->bID]);
        parent::delete();
    }
    ...
    public function duplicate($newBID)
    {
        ...
        foreach ($addCards_items as $addCards_item) {
            ...
            $db->insert('btClientlogoSliderAddCardsEntries', $addCards_item);
        }
    ...
    protected function addEdit()
    {
        ...
        $al->register('css', 'repeatable-ft.form', 'blocks/clientlogo_slider/css_form/repeatable-ft.form.css', [], $this->pkg);
        $al->register('javascript', 'handlebars', 'blocks/clientlogo_slider/js_form/handlebars-v4.0.4.js', [], $this->pkg);
        $al->register('javascript', 'handlebars-helpers', 'blocks/clientlogo_slider/js_form/handlebars-helpers.js', [], $this->pkg);
    ...
    public function save($args)
    {
        ...
        $rows = $db->fetchAll('SELECT * FROM btClientlogoSliderAddCardsEntries WHERE bID = ? ORDER BY sortOrder', [$this->bID]);
        ...
        if (!empty($addCards_items)) {
            ...
            foreach ($addCards_items as $addCards_item) {
                ...
                if (isset($addCards_item['externalLogoUrl']) && trim($addCards_item['externalLogoUrl']) != '') {
                    $data['externalLogoUrl'] = trim($addCards_item['externalLogoUrl']);
                } else {
                    $data['externalLogoUrl'] = null;
                }
                if (isset($rows[$i])) {
                    ...
                    switch ($type) {
                        case 'update':
                            foreach ($values as $id => $data) {
                                $db->update('btClientlogoSliderAddCardsEntries', $data, ['id' => $id]);
                            }
                            break;
                        case 'insert':
                            foreach ($values as $data) {
                                $db->insert('btClientlogoSliderAddCardsEntries', $data);
                            }
                            break;
                        case 'delete':
                            foreach ($values as $value) {
                                $db->delete('btClientlogoSliderAddCardsEntries', ['id' => $value]);
                            }
                            break;
                    }
                }
            ...
            if ($addCardsEntriesErrors == 0) {
                foreach ($args['addCards'] as $addCards_k => $addCards_v) {
                    if (is_array($addCards_v)) {
                        ...
                        if (in_array("externalLogoUrl", $this->btFieldsRequired['addCards']) && (!isset($addCards_v['externalLogoUrl']) || trim($addCards_v['externalLogoUrl']) == "")) {
                            $e->add(t("The %s field is required (%s, row #%s).", t("External Logo Url"), t("add cards"), $addCards_k));
                        }
                    } else {
