...
<div role="tabpanel" class="tab-pane show active" id="form-basics-<?php echo $identifier_getString; ?>">
    ...
    <?php echo $form->label($view->field('bgColor'), t("back color")); ?>
...
<?php $repeatable_container_id = 'btClientlogoSlider-addCards-container-' . $identifier_getString; ?>
...
<concrete-file-input file-id="{{ image }}"
    ...
    <label for="<?php echo $view->field('addCards'); ?>[{{id}}][icon]" class="control-label"><?php echo t("icon(svg)"); ?></label>
    <?php echo isset($btFieldsRequired['addCards']) && in_array('icon', $btFieldsRequired['addCards']) ? '<small class="required">' . t('Required') . '</small>' : null; ?>
    <div id="addCards-icon-image-{{id}}" class="ccm-file-selector ft-image-addCards-icon-file-selector">
<concrete-file-input file-id="{{ icon }}"
                                                     choose-text="<?php echo t('Choose Image'); ?>"
                                                     input-name="<?php echo $view->field('addCards'); ?>[{{id}}][icon]">
                                </concrete-file-input>
</div>
</div>            <div class="form-group">
    <label for="<?php echo $view->field('addCards'); ?>[{{id}}][externalLogoUrl]" class="control-label"><?php echo t("External Logo Url"); ?></label>
    <?php echo isset($btFieldsRequired['addCards']) && in_array('externalLogoUrl', $btFieldsRequired['addCards']) ? '<small class="required">' . t('Required') . '</small>' : null; ?>
    <input name="<?php echo $view->field('addCards'); ?>[{{id}}][externalLogoUrl]" id="<?php echo $view->field('addCards'); ?>[{{id}}][externalLogoUrl]" class="form-control" type="text" value="{{ externalLogoUrl }}" maxlength="255" />
</div></div>
...
<script type="text/javascript">
    Concrete.event.publish('btClientlogoSlider.addCards.edit.open', {id: '<?php echo $repeatable_container_id; ?>'});
