Concrete.event.bind('btAccordionBlock.accordionsItem.edit.open', function (options, settings) {
    ...
    blockDesignerEditor('#' + ($(newField).find('textarea.ft-accordionsItem-accordionContent').attr('id')).replace(/(:|\.|\[|\]|,|=|@)/g, "\\$1"));
    };
        ...
        if (deleteIt === true) {
            ...
            $(sortableItem).slideUp(slideSpeed, function () {
                if(typeof CKEDITOR !== 'undefined'){
                    var blockDesignerEditorInstance = CKEDITOR.instances[$(sortableItem).find('textarea.ft-accordionsItem-accordionContent').attr('id')];
