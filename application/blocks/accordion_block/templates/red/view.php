...
<section <?php if (isset($bgColor) && trim($bgColor) != "") { ?>style="background-color: <?php echo h($bgColor); ?>;" <?php } ?> class="<?php if (isset($removePaddingTop) && trim($removePaddingTop) == 1) { ?>pt-0<?php } ?> <?php if (isset($removePaddingBottom) && trim($removePaddingBottom) == 1) { ?>pb-0<?php } ?> section accordion-block accordion-block--red xxl:px-[10rem] xl:px-[6rem] md:px-[4rem] px-[2rem] xl:py-[10rem] md:py-[8rem] py-[5rem] text-white" id="whychanneline">
    <div class="flex  flex-col lg:flex-row justify-between gap-12 lg:gap-[8.6rem]">
        ...
        <?php if (!empty($accordionsItem_items)) { ?>
            ...
            <div class="acc-container lg:w-3/4 w-full ">
                <?php foreach ($accordionsItem_items as $accordionsItem_item_key => $accordionsItem_item) { ?>
                    <div tabindex="0" role="button" aria-label="accordion <?php echo h($accordionsItem_item["accordionTitle"]); ?>" class="acc active:bg-red-berry active:text-white group lg:opacity-0 lg:translate-x-[4rem] bg-white md:py-[3.5rem] py-[3rem] md:px-[4rem] px-[2rem]  mb-[2rem] transition-colors duration-1000 ease-out hover:bg-red-berry hover:text-white group">
